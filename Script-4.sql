...
sales_data AS (
    SELECT 
        ...
        SUM(CASE 
            WHEN uos.max_dispensing_status = 'completed' and uos.order_guid not IN (SELECT distinct order_guid FROM started_orders) THEN uos.total_payment  ELSE 0 END
        ) AS total_sale,
        COUNT(CASE 
            WHEN uos.max_dispensing_status = 'completed' AND uos.order_guid NOT IN (SELECT order_guid FROM started_orders) THEN 1 ELSE NULL END
